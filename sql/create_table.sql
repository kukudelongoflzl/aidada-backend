...
(
    ...
    appType  tinyint   default 0                        not null comment '应用类型（0-得分类 1测评类）',
    scoringStrategy  tinyint   default 0                 not null comment '评分策略（0-自定义 AI）',
    reviewStatus   int default 0                        not null comment '审核状态0-待审核 1-审核通过 2-审核不通过',
    reviewMessage  varchar(512)                         null comment '审核信息',
    reviewerId   bigint                                   null comment '审核者ID',
...
(
    ...
    resultScoreRange  int                            null comment '结果得分范围，如80，表示80以及以上的分数命中此结果',
    appId           bigint                          not null comment '应用ID',
    userId       bigint                                not null comment '创建用户 id',
    createTime   datetime     default CURRENT_TIMESTAMP not null comment '创建时间',
    updateTime   datetime     default CURRENT_TIMESTAMP not null on update CURRENT_TIMESTAMP comment '更新时间',
...
(
    ...
    appType  tinyint   default 0                        not null comment '应用类型（0-得分类 1测评类）',
    scoringStrategy  tinyint   default 0                 not null comment '评分策略（0-自定义 AI）',
    choices text                                         null comment '用户答案（JSON）',
    resultId bigint                                      null comment '评分结果ID',
    resultName  varchar(128)                            not null comment '结果名称，如物流师',
    resultDesc  text                           null comment '结果描述',
    resultPicture  varchar(1024)                            null comment '结果图片',
    resultProp  varchar(128)                            null comment '结果属性集合 JSON,如[I,S,T,J]',
    resultScore  int                            null comment '结果得分范围，如80，表示80以及以上的分数命中此结果',
    userId       bigint                                not null comment '创建用户 id',
    createTime   datetime     default CURRENT_TIMESTAMP not null comment '创建时间',
    updateTime   datetime     default CURRENT_TIMESTAMP not null on update CURRENT_TIMESTAMP comment '更新时间',
    isDelete     tinyint      default 0                 not null comment '是否删除',
